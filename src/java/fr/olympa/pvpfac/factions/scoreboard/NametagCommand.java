...
public class NametagCommand implements CommandExecutor, TabExecutor {
	...
	private void cmdConvert(CommandSender sender, String[] args) {
		if (isNotPermissed(sender, "nametagedit.convert")) {
			return;
		}
		if (args.length != 4) {
	...
	private void cmdGroups(CommandSender sender, String[] args) {
		if (isNotPermissed(sender, "nametagedit.groups")) {
			return;
		}
		if (args.length < 2) {
	...
	private void cmdPlayer(CommandSender sender, String[] args) {
		if (args.length == 3) {
			...
			if (isNotPermissed(sender, "nametagedit.clear.self")) {
				return;
			}
			...
			case "prefix":
			case "suffix":
				if (isNotPermissed(sender, "nametagedit.edit.self")) {
					return;
				}
	...
	private void cmdPriority(CommandSender sender, String[] args) {
		if (isNotPermissed(sender, "nametagedit.priority")) {
			return;
//        if (args.length == 0) {
...
//        abstractConfig.save(groupData.toArray(new GroupData[groupData.size()]));
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (isNotPermissed(sender, "nametagedit.use")) {
			return false;
		}
		if (args.length < 1) {
			sendUsage(sender);
