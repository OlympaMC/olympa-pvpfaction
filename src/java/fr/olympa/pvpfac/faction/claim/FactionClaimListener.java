...
public class FactionClaimListener implements Listener {
	...
	public void onDamageByEntity(final EntityDamageByEntityEvent e) {
		...
		final FactionPlayer damagerFactionPlayer = AccountProviderAPI.getter().get(damager.getUniqueId());
		...
		if (target.getType() == EntityType.PLAYER) {
			final FactionPlayer targetFactionPlayer = AccountProviderAPI.getter().get(target.getUniqueId());
	...
	private boolean isActionCancelled(final Location loc, final Player p, final Cancellable event, final Function<FactionClaimPermLevel, Boolean> method, final String denyMessage) {
		...
		if (!method.apply(PvPFaction.getInstance().getClaimsManager().ofChunk(loc.getChunk()).getPlayerPerm(AccountProviderAPI.getter().get(p.getUniqueId())))) {
	...
	public void onPlayerBucketEmpty(final PlayerBucketEmptyEvent e) {
		...
		final Faction faction = ((FactionPlayer) AccountProviderAPI.getter().get(p.getUniqueId())).getClan();
		...
		if (
			faction != null && Bukkit.getOnlinePlayers().stream().anyMatch(
				pl ->
					...
					faction.isAlly(((FactionPlayer) AccountProviderAPI.getter().get(pl.getUniqueId())).getClan())
			)
		) {
